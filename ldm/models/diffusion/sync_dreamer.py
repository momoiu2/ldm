...
from ldm.models.diffusion.glp_models.model import GLPDepth
...
class SpatialVolumeNet(nn.Module):
    ...
    def construct_spatial_volume(self, x, t_embed, v_embed, target_poses, target_Ks):
        ...
        device = x.device
        # 使用 torch.linspace 创建了一个均匀间隔的浮点数序列 spatial_volume_verts，表示空间体素坐标。
        ...
        for ni in range(0, N):
            ...
            # 调用 get_warp_coordinates 函数，获取在三维空间中投影的坐标。
            ...
            # 使用双线性插值在输入图像 x_ 上进行网格采样，以获取在三维空间中的投影特征。
...
class SyncMultiviewDiffusion(pl.LightningModule):
    def __init__(self, unet_config, scheduler_config, glp_config,
        ...
        self.Depth_model = GLPDepth(glp_config.params)
        checkpoint = torch.load("/data2/xzq/xzq/SyncDreamer/SyncDreamer/nyudepthv2_swin_base.ckpt")
        # print(checkpoint.keys)
        # 获取模型状态字典
        # model_state_dict = checkpoint['model_state_dict']

        # 将模型的权重加载到已初始化的模型中
        self.Depth_model.load_state_dict(checkpoint, strict=True)
    ...
    def get_viewpoint_embedding(self, batch_size, elevation_ref):
        ...
        @param elevation_ref: 参考的仰角，这里是一个张量，形状为 (B,)，B 是批处理的大小
        @return: 返回视点嵌入，形状为 (B, N, 4)，其中 N 是视点的数量
        """
        azimuth_input = self.azimuth[0].unsqueeze(0)  # 参考的方位角，这里假设是一个张量，形状为 (1,)
        azimuth_target = self.azimuth  # 所有视点的方位角，这里假设是一个张量，形状为 (N,)
        elevation_input = -elevation_ref  # 注意，这里使用了负的仰角，这可能是特定情况下的处理选择
        elevation_target = -np.deg2rad(30)  # 目标仰角，这里使用了一个固定的值 -30 度，并转换为弧度

        d_e = elevation_target - elevation_input  # 仰角差，形状为 (B, 1)，表示每个样本的仰角差
        N = self.azimuth.shape[0]  # 视点的数量
        B = batch_size  # 批处理的大小

        # 复制仰角差和方位角差，以构建视点嵌入
        d_e = d_e.unsqueeze(1).repeat(1, N)  # 在第二个维度上进行复制，形状变为 (B, N)
        d_a = azimuth_target - azimuth_input  # 方位角差，形状为 (B, N)
        d_a = d_a.unsqueeze(0).repeat(B, 1)  # 在第一个维度上进行复制，形状变为 (B, N)
        d_z = torch.zeros_like(d_a)  # 高度差，形状与方位角差相同，都是 (B, N)

        # 将仰角差、sin(方位角差)、cos(方位角差)、高度差堆叠在一起，形成视点嵌入
        embedding = torch.stack([d_e, torch.sin(d_a), torch.cos(d_a), d_z], -1)  # 形状为 (B, N, 4)
    ...
    def training_step(self, batch):
        ...
        # clip_embed为融合了目标target_index图像视点信息的嵌入，volume_feats表示目标视锥体体积的特征表示的多分辨率特征图，x_concat表示原始输入图像
    ...
    def configure_optimizers(self):
        ...
        paras.append({"params": self.Depth_model.parameters(), "lr": lr}, )
